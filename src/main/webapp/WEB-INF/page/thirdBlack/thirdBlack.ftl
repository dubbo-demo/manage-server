				...
				<table class="table table-striped table-bordered table-hover">
					...
					<tbody>
						...
						<tr class="odd gradeX" id="${item.id}">
							...
							<td>${item.srcOrg!'' }</td>
							<td><#if item.isReject??>
								${(item.isReject==0)?string('否','是') } </#if></td>
							<td>${item.rejectReason!'' }</td>
							<td><div style="width:330px;word-break:break-all">${item.respMessage!}</div></td>
							<td><#if item.createTime??>${item.createTime?date}</#if></td>
							<td>
							<shiro:hasPermission name="thirdBlack:delete">
							<a href="javascript:void(0)"	onclick="deleteById('${item.id}',event)">删除</a>
							</shiro:hasPermission>
							</td>
						</tr>
						</#list>
...
<script>
	...
	function impInfo() {
		...
		var url = "${ctx}/thirdBlack/impInfo.htm";
		$("#frm").attr("action", url);
		$("#frm").submit();
	}
	//验证信息
